...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const defaultNetProfitRevenue = @json($netProfitRevenue);
            ...
            console.log(defaultCompletedOrder);
            console.log(defaultCancelOrder);
            ...
            if (defaultLabels && defaultRevenue) {
                ...
                updateNetProfit(defaultNetProfitRevenue, 'Lợi nhuận thuần');
                ...
                if (filterValue == '14day') {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response.netProfitRevenue !== undefined) {
                                updateNetProfit(response.netProfitRevenue, filterValue);
                            }
                ...
                if (filter && date) {
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response.netProfitRevenue !== undefined) {
                                updateNetProfit(response.netProfitRevenue, filter);
                            }
                            if (response.pendingOrder !== undefined) {
        ...
        // Lợi nhuận thuần 
        function updateNetProfit(netProfit, filter) {
            if (!netProfit) {
                $('#netProfitLabel').text('Lợi nhuận thuần');
                $('#netProfit').text('0 VNĐ');
            }
            ...
            switch (filter) {
                case 'day':
                    label = 'Lợi nhuận thuần theo ngày';
                    break;
                case 'month':
                    label = 'Lợi nhuận thuần theo tháng';
                    break;
                case 'year':
                    label = 'Lợi nhuận thuần theo năm';
                    break;
                case '14day':
                    label = 'Lợi nhuận thuần 14 ngày gần nhất';
                    break;
                default:
                    label = 'Lợi nhuận thuần';
                    break;
            }
            $('#netProfitLabel').text(label);
            $('#netProfit').text(netProfit.toLocaleString() + ' VNĐ');
        }
